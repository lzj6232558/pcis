...
$(function () {
    ...
    var methodObject = {
        ...
        back:function () {
            ...
            if (!row) {
                $.messager.alert('提示信息', '大哥,请选择一条需要退回申请的保单',"info");
                return;
            }

            //修改保单状态为3
            $.get("/policy/changeState.do", {state: 3,id:row.id}, function (data) {
                if (data.success) {
                    $.messager.alert('我的消息', '成功退回', 'info', function () {
                        $("#tempoary_datagrid").datagrid("reload");
                    });
                } else {
                    $.messager.alert('我的消息', '退回申请失败', 'erro');
                }
            }, "json")
        },
        //审核
        pass:function () {
            var row = $("#tempoary_datagrid").datagrid("getSelected");
            if (!row) {
                $.messager.alert('提示信息', '大哥,请选择一条需要审核的保单',"info");
                return;
            }

            //修改保单状态为2,已审核待缴费
            $.get("/policy/audit.do", {state: 2,id:row.id}, function (data) {
                if (data.success) {
                    $.messager.alert('我的消息', '审核成功', 'info', function () {
                        $("#tempoary_datagrid").datagrid("reload");
                    });
                } else {
                    $.messager.alert('我的消息', '审核失败', 'erro');
                }
            }, "json")
        },
